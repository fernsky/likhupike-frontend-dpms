...
import { CooperativeResponse, CooperativeType, CooperativeStatus } from '../../types';
import { CooperativeActions, CooperativeSearchActions, CooperativeUIActions } from '../../store/actions';
...
export class CooperativeListPageComponent implements OnInit, OnDestroy {
  ...
  constructor(private store: Store) {
    this.cooperatives$ = this.store.select(fromCooperatives.selectAllCooperatives);
    this.loading$ = this.store.select(fromCooperatives.selectCooperativesLoading);
    this.totalItems$ = this.store.select(fromCooperatives.selectSearchTotalResults);
    ...
    this.activeSearchMethod$ = this.store.select(fromCooperatives.selectActiveSearchMethod);
  }
  ...
  loadDefaultCooperatives(): void {
    this.store.dispatch(CooperativeActions.loadCooperatives({
      page: 0,
      size: 10
    }));
  }
  ...
  onSearch(searchTerm: string): void {
    if (searchTerm.trim()) {
      this.store.dispatch(CooperativeSearchActions.searchByName({
  ...
  onFilterByStatus(status: CooperativeStatus): void {
    this.store.dispatch(CooperativeSearchActions.getByStatus({
      status,
      page: 0,
      size: 10
    }));
  ...
  onPageChange(event: PageEvent): void {
    this.store.dispatch(CooperativeSearchActions.setSearchPage({
