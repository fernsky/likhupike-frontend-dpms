...
export class CooperativeTranslationsComponent implements OnInit, OnChanges {
  ...
  constructor(
    ...
    this.translations$ = this.store.select(fromCooperatives.selectAllTranslations);
    this.loading$ = this.store.select(fromCooperatives.selectTranslationsLoading);
  }
  ...
  loadTranslations(): void {
    this.store.dispatch(CooperativeTranslationActions.loadTranslations({
  ...
  getAvailableLocales(): string[] {
    ...
    return this.cooperative.translations 
      ? this.availableLocales.filter(locale => 
  ...
  openCreateTranslationDialog(): void {
    ...
    dialogRef.afterClosed().subscribe((result: CreateCooperativeTranslationDto) => {
      if (result) {
        this.store.dispatch(CooperativeTranslationActions.createTranslation({
          cooperativeId: this.cooperative.id,
          translation: result
        }));
      }
  ...
  openEditTranslationDialog(translation: CooperativeTranslationResponse): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.store.dispatch(CooperativeTranslationActions.updateTranslation({
          cooperativeId: this.cooperative.id,
  ...
  confirmDeleteTranslation(translation: CooperativeTranslationResponse): void {
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: this.transloco.translate('cooperative.dialogs.deleteTranslationTitle'),
        message: this.transloco.translate('cooperative.dialogs.deleteTranslationMessage', {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this.store.dispatch(CooperativeTranslationActions.deleteTranslation({
          cooperativeId: this.cooperative.id,
          translationId: translation.id
        }));
      }
  ...
  updateTranslationStatus(translation: CooperativeTranslationResponse, status: ContentStatus): void {
    this.store.dispatch(CooperativeTranslationActions.updateTranslationStatus({
      cooperativeId: this.cooperative.id,
      translationId: translation.id,
      status: status
    }));
  ...
  getStatusColor(status: ContentStatus): string {
    switch (status) {
      case ContentStatus.PUBLISHED: return 'primary';
      case ContentStatus.DRAFT: return 'warn';
      case ContentStatus.PENDING_REVIEW: return 'accent';
      case ContentStatus.ARCHIVED: return '';
      default: return '';
    }
  }
}
